...
export async function skillFiles(skillsRoot, skill) {
  ...
  await walk(root);
  for (const required of ['SKILL.md', 'assets/template.json', 'assets/eval-prompts.json', 'agents/openai.yaml', 'references/deckforge-format.txt']) if (!files.some(file => file.name === `skills/${skill}/${required}`)) throw new Error(`Skill bundle is incomplete: ${required}.`);
  return files;
}
